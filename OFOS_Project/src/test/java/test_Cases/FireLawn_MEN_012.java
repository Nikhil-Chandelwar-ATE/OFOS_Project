...
public class FireLawn_MEN_012 extends Base_Test{

	@Test
	public void discountPercentageOfProduct() throws Throwable {
		...
		if (tp.getProducts().size()>0) {
			
			Assert.assertEquals(tp.getProductPercentage().isDisplayed(), true);
			test.log(LogStatus.PASS, test.addScreenCapture(getWebelementScreenshot(tp.getProductPercentage())));
		}
		else {
